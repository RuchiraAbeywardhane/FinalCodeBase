...
unique_subjects = sorted(set(allSID))
print(f"Running LOSO over {len(unique_subjects)} subjects ...")

loso_window_rows = []
loso_trial_rows  = []
loso_subj_scores = {}

for hold_sid in unique_subjects:
    loso_tr_mask = (allSID != hold_sid)
    loso_te_mask = (allSID == hold_sid)

    loso_trF_raw = allF_raw[loso_tr_mask]
    loso_trY     = allY[loso_tr_mask]
    loso_teF_raw = allF_raw[loso_te_mask]
    loso_teY     = allY[loso_te_mask]
    loso_teTK    = allTK[loso_te_mask]

    if len(loso_trY) == 0 or len(loso_teY) == 0:
        print(f"  [LOSO] subject {hold_sid}: insufficient data -- skipping.")
        continue

    # VarianceThreshold: fit on train subjects only
    loso_vt = VarianceThreshold(threshold=0.001)
    loso_vt.fit(loso_trF_raw)
    loso_trF_sel = loso_vt.transform(loso_trF_raw)
    loso_teF_sel = loso_vt.transform(loso_teF_raw)

    # Global z-score: stats from train subjects only
    loso_mu = loso_trF_sel.mean(axis=0)
    loso_sd = loso_trF_sel.std(axis=0)
    loso_sd = np.where(loso_sd < 1e-8, 1.0, loso_sd)

    loso_trF_n = np.clip(safe_array((loso_trF_sel - loso_mu) / loso_sd), -10, 10)
    loso_teF_n = np.clip(safe_array((loso_teF_sel - loso_mu) / loso_sd), -10, 10)
    loso_nfeat = loso_trF_n.shape[1]

    # MI feature selection: fit on train subjects only
    sub_idx_loso = (
        np.random.RandomState(42).choice(len(loso_trF_n), 2000, replace=False)
        if len(loso_trF_n) > 2000 else np.arange(len(loso_trF_n))
    )
    loso_mi = mutual_info_classif(
        loso_trF_n[sub_idx_loso], loso_trY[sub_idx_loso],
        random_state=42, n_neighbors=5
    )
    loso_feat_idx = np.argsort(-loso_mi)[:min(FINAL_K, loso_nfeat)]

    # Train LDA
    loso_clf = LinearDiscriminantAnalysis(solver='lsqr', shrinkage=FINAL_SH)
    try:
        loso_clf.fit(loso_trF_n[:, loso_feat_idx], loso_trY)
    except Exception as e:
        print(f"  [LOSO] subject {hold_sid}: LDA fit failed ({e}) -- skipping.")
        continue

    # Predict
    loso_probs = loso_clf.predict_proba(loso_teF_n[:, loso_feat_idx])
    loso_preds = np.argmax(loso_probs, axis=1)
    win_acc = float((loso_preds == loso_teY).mean())

    # Collect window + trial rows
    trial_correct = []
    for tkey in sorted(set(loso_teTK)):
        m = (loso_teTK == tkey)
        probs_t  = loso_probs[m]
        preds_t  = loso_preds[m]
        true_lbl = int(loso_teY[m][0])

        for r in range(len(preds_t)):
            loso_window_rows.append({
                "subject":         hold_sid,
                "trial_key":       tkey,
                "window_idx":      r,
                "true_idx":        true_lbl,
                "true_label":      IDX_TO_LABEL[true_lbl],
                "pred_idx":        int(preds_t[r]),
                "pred_label":      IDX_TO_LABEL[int(preds_t[r])],
        ...
        trial_correct.append(int(pi == true_lbl))
        loso_trial_rows.append({
            ...
            "prob_NEUTRAL":     float(mp[0]),
            "prob_ENTHUSIASM":  float(mp[1]),
            "prob_SADNESS":     float(mp[2]),
            "prob_FEAR":        float(mp[3]),
        })

    trial_acc = float(np.mean(trial_correct)) if trial_correct else 0.0
    loso_subj_scores[hold_sid] = {
        "window_acc": win_acc,
        "trial_acc":  trial_acc,
        "n_windows":  int(loso_te_mask.sum()),
        "n_trials":   len(trial_correct),
    }
    print(f"  Subject {hold_sid:>4s}:  win_acc={win_acc:.3f}  trial_acc={trial_acc:.3f}"
          f"  ({loso_te_mask.sum()} windows, {len(trial_correct)} trials)")

# Save LOSO CSVs
loso_win_df   = pd.DataFrame(loso_window_rows)
loso_trial_df = pd.DataFrame(loso_trial_rows)
OUT_LOSO_WINDOW_CSV = "/kaggle/working/loso_window_predictions.csv"
OUT_LOSO_TRIAL_CSV  = "/kaggle/working/loso_trial_predictions.csv"
loso_win_df.to_csv(OUT_LOSO_WINDOW_CSV,  index=False)
loso_trial_df.to_csv(OUT_LOSO_TRIAL_CSV, index=False)
print(f"\nSaved -> {OUT_LOSO_WINDOW_CSV}")
print(f"Saved -> {OUT_LOSO_TRIAL_CSV}")

# LOSO window-level report
if len(loso_win_df) > 0:
    loso_y_true_w = loso_win_df["true_idx"].astype(int).values
    loso_y_pred_w = loso_win_df["pred_idx"].astype(int).values
    print("\n" + "="*60)
    print("LOSO  -  WINDOW-LEVEL RESULTS (all subjects pooled)")
    print("="*60)
    print(f"Overall window accuracy  : {(loso_y_true_w == loso_y_pred_w).mean():.4f}")
    print(f"Mean per-subject win acc : {np.mean([v['window_acc'] for v in loso_subj_scores.values()]):.4f}")
    print(classification_report(loso_y_true_w, loso_y_pred_w,
          target_names=[IDX_TO_LABEL[i] for i in range(NUM_CLASSES)], zero_division=0))
    print("Confusion matrix:")
    print(confusion_matrix(loso_y_true_w, loso_y_pred_w, labels=list(range(NUM_CLASSES))))

# LOSO trial-level report
if len(loso_trial_df) > 0:
    loso_y_true_t = loso_trial_df["true_idx"].astype(int).values
    loso_y_pred_t = loso_trial_df["trial_pred_idx"].astype(int).values
    print("\n" + "="*60)
    print("LOSO  -  TRIAL-LEVEL RESULTS (all subjects pooled)")
    print("="*60)
    print(f"Overall trial accuracy     : {(loso_y_true_t == loso_y_pred_t).mean():.4f}")
    print(f"Mean per-subject trial acc : {np.mean([v['trial_acc'] for v in loso_subj_scores.values()]):.4f}")
    print(classification_report(loso_y_true_t, loso_y_pred_t,
          target_names=[IDX_TO_LABEL[i] for i in range(NUM_CLASSES)], zero_division=0))
    print("Confusion matrix:")
    print(confusion_matrix(loso_y_true_t, loso_y_pred_t, labels=list(range(NUM_CLASSES))))

    print("\nPer-subject summary:")
    print(f"{'Subject':>8}  {'Win Acc':>8}  {'Trial Acc':>10}  {'#Windows':>9}  {'#Trials':>8}")
    print("-" * 52)
    for sid, sc in sorted(loso_subj_scores.items()):
        print(f"{sid:>8}  {sc['window_acc']:>8.3f}  {sc['trial_acc']:>10.3f}"
              f"  {sc['n_windows']:>9}  {sc['n_trials']:>8}")
