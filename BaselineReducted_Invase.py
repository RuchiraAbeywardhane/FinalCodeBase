...
def load_subject_baselines(data_root, baseline_keyword="BASELINE"):
    ...
    for subj in sorted(os.listdir(data_root)):
        ...
        try:
            ...
            baselines[subj] = np.mean(sig, axis=0).astype(np.float32)
...
if subject_baselines:
    global_baseline = np.mean(np.stack(list(subject_baselines.values()), axis=0), axis=0)
...
for subj in sorted(os.listdir(EMOGNITION_ROOT)):
    ...
    for eeg_json in cleaned_files:
        ...
        baseline_vec = subject_baselines.get(sid, global_baseline)
        if baseline_vec is not None:
            eeg = apply_baseline_reduction(
                eeg.T, baseline_vec, eps=BASELINE_EPS, method=BASELINE_METHOD
            ).T  # back to (C, T)
            eeg = safe_array(eeg)
